...
# import os  # For handling folder creation
...
# output_folder = "output_for_app_forecasting"
...
#     # Save the figure
#     plt.savefig(f"{output_folder}/install_rating_trends.png", bbox_inches='tight')
#     plt.close()
...
#         # Save the figure
#         plt.savefig(f"{output_folder}/stacked_area_chart_{i+1}_{i+num_plots}.png", bbox_inches='tight')
#         plt.close()
...
#     # Save the figure
#     plt.savefig(f"{output_folder}/arima_forecast.png", bbox_inches='tight')
#     plt.close()
...
# # --- Visualization: Braided Graph for App Growth by Category ---
# def plot_braided_graph():
#     # Grouping by category and date to create braided graph
#     df_category_grouped = df.groupby([df['Released'], 'Category']).agg({'Total Installs': 'sum'}).reset_index()

#     # Pivoting for braided graph
#     df_pivot = df_category_grouped.pivot_table(index='Released', columns='Category', values='Total Installs', aggfunc='sum').fillna(0)

#     # Plotting the braided graph (line graph with multiple categories)
#     fig, ax = plt.subplots(figsize=(10, 6))

#     # Plot each category as a line on the same graph
#     for category in df_pivot.columns:
#         ax.plot(df_pivot.index, df_pivot[category], label=category, linewidth=2)

#     # Set title and labels
#     ax.set_title('App Growth by Category (Braided Graph)', fontsize=14)
#     ax.set_xlabel('Date', fontsize=12)
#     ax.set_ylabel('Total Installs', fontsize=12)
...
#     # Save the figure
#     plt.savefig(f"{output_folder}/braided_graph.png", bbox_inches='tight')
#     plt.close()
...
# --- Visualization: Braided Graphs for App Growth by Category ---
def plot_braided_graph():
    # Grouping by category and date to create braided graph
    df_category_grouped = df.groupby([df['Released'], 'Category']).agg({'Total Installs': 'sum'}).reset_index()
    ...
    for i in range(0, num_categories, num_plots):
        ...
        # Create braided (overlapping) line plots for each category
        for category in subset_categories:
            ax.plot(df_subset.index, df_subset[category], label=category)
        ...
        # Save the figure
        plt.savefig(f"{output_folder}/braided_graph_{i+1}_{i+num_plots}.png", bbox_inches='tight')
        plt.close()
